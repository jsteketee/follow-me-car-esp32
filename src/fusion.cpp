...
static Pose     _fusedPose          = { 0.0f, -1.0f, 0 };
...
static void correct_bearing(float relAngle, float imuYaw, float r) {
    ...
    // Sign-flip detector: spikes when consecutive innovations reverse direction (erratic sensor),
    // stays near zero when innovations are consistently signed (genuine tag movement).
    static float _prevInnov = 0.0f;
    float jitter = fmaxf(0.0f, -innov * _prevInnov);
    _prevInnov   = innov;
    _innovEwma = fminf(rtConfig.fusionInnovEwmaAlpha * jitter + (1.0f - rtConfig.fusionInnovEwmaAlpha) * _innovEwma,
...
void fusion_update() {
    ...
    if (_lastUpdateMs > 0) {
        ...
        _bearingP += rtConfig.fusionQBearingPerSec * dt;
    }
    _lastUpdateMs = now;
    ...
    if (cam.timestamp != _lastCameraTimestamp) {
        ...
        if (cam.found) {
            ...
            correct_bearing(camAngle, imu.yaw, FUSION_KALMAN_R_CAMERA);
            _fusedPose.timestamp = now;
            camUpdated = true;
        }
    }
    ...
    if (uwbUpdated || camUpdated) {
        ESP_LOGI(TAG, "bearing=%.1f°  angle=%.1f°  dist=%.0fcm  unc=%.1f  src=%s",
            _bearingDeg, _fusedPose.angle, _fusedPose.distanceCm,
